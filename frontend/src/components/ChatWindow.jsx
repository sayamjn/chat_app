...
const ChatWindow = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        setError('Failed to fetch users. Please refresh and try again.');
      }
    };
  ...
  useEffect(() => {
    ...
    socket.on('newMessage', ({ senderId, content }) => {
      console.log('New message received:', { senderId, content });
  ...
  useEffect(() => {
    if (selectedUser) {
      const fetchMessages = async () => {
        try {
          console.log(`Fetching messages with ${selectedUser.username} (ID: ${selectedUser._id})`);
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      console.log(`Sending message to ${selectedUser.username}: ${newMessage}`);
      ...
      <div className="flex-1 flex overflow-hidden max-w-7xl mx-auto w-full">
        {isMobileView && !showUserList && (
          <button
            ...
            className="fixed z-10 left-4 bottom-20 bg-blue-500 text-white p-3 rounded-full shadow-lg"
          >
            <FaUsers />
          </button>
        )}
              ...
              <div className="p-4 border-t border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800">
                <form onSubmit={handleSendMessage} className="flex">
                  ...
                  <button
                    ...
                    className="bg-gradient-to-r from-blue-500 to-purple-600 text-white px-4 py-3 rounded-r-lg hover:from-blue-600 hover:to-purple-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 flex items-center justify-center"
                  >
                    <FaPaperPlane />
                  </button>
                </form>
